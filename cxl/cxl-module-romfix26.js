...
function parseImageConfig(params) {
    ...
    const defaultHpcKernelUrl = /^asplos\.dev$/i.test(location.hostname)
        ? `https://raw.githubusercontent.com/vickiegpt/vickiegpt.github.io/main/cxl/images/alpine-x86_64/bzImage-cxl-dax?v=${hpcKernelVersion}`
        : new URL(`/cxl/images/alpine-x86_64/bzImage-cxl-dax?v=${hpcKernelVersion}`, location.href).href;
    ...
    const biosUrl = firstUrlParam(params, ['bios_url', 'microvm_bios_url'])
        || new URL('/cxl/images/alpine-x86_64/bios-microvm.bin', location.href).href;
    const defaultPcBiosUrl = /^asplos\.dev$/i.test(location.hostname)
        ? `https://raw.githubusercontent.com/vickiegpt/vickiegpt.github.io/main/cxl/images/alpine-x86_64/bios-256k.bin?v=${pcBiosVersion}`
        : new URL(`/cxl/images/alpine-x86_64/bios-256k.bin?v=${pcBiosVersion}`, location.href).href;
    ...
    const defaultEfiE1000RomUrl = /^asplos\.dev$/i.test(location.hostname)
        ? `https://raw.githubusercontent.com/vickiegpt/vickiegpt.github.io/main/cxl/images/alpine-x86_64/efi-e1000.rom?v=${efiE1000RomVersion}`
        : new URL(`/cxl/images/alpine-x86_64/efi-e1000.rom?v=${efiE1000RomVersion}`, location.href).href;
...
const CXL_WEB_CONFIG = (() => {
    ...
    const qemuCoreNames = new Set(['fast', 'may17', 'fpcast', 'build', 'safe', 'relfix', 'o3-clean']);
    ...
    if (fastShellMicrovm && !kernelExplicit) {
        image.kernelUrl = new URL('/cxl/images/hetgpu-webgpu/load-kernel.data', location.href).href;
    }
    ...
    return {
        ...
        type2CacheBytes,
        assetVersion: ({
            fast: '20260522-may17-stable1',
            may17: '20260522-may17-stable1',
            fpcast: '20260512-numfix',
            build: '20260516-build',
            safe: '20260512-safe',
            relfix: '20260512-relfix',
            'o3-clean': '20260512-o3-clean'
        })[qemuCore] || '20260522-may17-stable1',
        assetBase: ({
            fast: '/cxl/images/alpine-x86_64-may17/',
            may17: '/cxl/images/alpine-x86_64-may17/',
            fpcast: '/cxl/images/alpine-x86_64-fpcast/',
            build: '/cxl/images/alpine-x86_64-build/',
            safe: '/cxl/images/alpine-x86_64-safe/',
            relfix: '/cxl/images/alpine-x86_64-relfix/',
            'o3-clean': '/cxl/images/alpine-x86_64-o3-clean/'
        })[qemuCore] || '/cxl/images/alpine-x86_64/',
        ...
        cxlmemsim: {
            ...
            pool: cxlmemsim.pool,
            size: cxlmemsimSize,
            workerUrl: '/cxl/cxlmemsim-pool-worker.js?v=20260522-may17-stable1',
            workerName: 'hetgpu-cxlmemsim-20260522-may17-stable1'
        },
        webgpuNative
    };
})();
...
Module['locateFile'] = function(path, prefix) {
    if (path === 'load-rom.data' || path.startsWith('load-rom.data?')) {
        return new URL(path, new URL('/cxl/images/alpine-x86_64/', location.href)).href;
    }
...
{
    const mainScriptUrl = new URL(CXL_WEB_CONFIG.qemuMainScript || 'out.js', new URL(CXL_WEB_CONFIG.assetBase, location.href));
