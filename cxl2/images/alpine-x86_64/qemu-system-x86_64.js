...
function instantiate_wasm() {
 ...
 for (var i = 0; i < import_vec_size / 4; i++) {
  const tableIndex = memory_v.getInt32(import_vec_begin + i * 4, true);
  const tableFunction = wasmTable.get(tableIndex);
  const importSig = importSigs.find(importInfo => importInfo.module === "helper" && importInfo.name === String(i));
  helper[i] = wrapWasmHelperImport(tableFunction, importSig && importSig.sig);
 }
 ...
 const fidx = addFunction((arg) => inst.exports.start(arg), "ii");
 return fidx;
}
