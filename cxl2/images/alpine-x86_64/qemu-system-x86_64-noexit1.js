...
["_cxl_browser_connect_js","_cxl_browser_disconnect_js","_cxl_browser_request_js","_cxl_browser_send_message_js","_cxl_browser_recv_message_js","___indirect_function_table","___em_lib_deps_libffi","_unbox_small_structs","_ffi_call_js","_ffi_closure_alloc_js","_ffi_closure_free_js","_ffi_prep_closure_loc_js","__emscripten_proxy_main","_main","onRuntimeInitialized"].forEach((prop) => {
...
var ASM_CONSTS = {
  9896140: () => { if (typeof console !== 'undefined') { console.log('[hetGPU WebGPU] backend selected'); } return 0; },  
 9896243: () => { if (typeof navigator !== 'undefined' && navigator.gpu) { return 1; } return 1; },  
 9896326: ($0) => { if (typeof console !== 'undefined') { console.log('[hetGPU WebGPU] module load, bytes=' + $0); } },  
 9896427: ($0, $1) => { if (typeof console !== 'undefined') { var n = UTF8ToString($1); console.log('[hetGPU WebGPU] get function module=' + $0 + ' name=' + n); } },  
 9896570: ($0, $1, $2, $3, $4, $5, $6, $7, $8) => { if (typeof console !== 'undefined') { var n = UTF8ToString($1); console.log('[hetGPU WebGPU] launch kernel=' + $0 + ' name=' + n + ' grid=(' + $2 + ',' + $3 + ',' + $4 + ')' + ' block=(' + $5 + ',' + $6 + ',' + $7 + ')' + ' shared=' + $8); } }
};
function cxl_browser_connect_js(device_ptr,pool_ptr,port,size_lo,size_hi) { function text(ptr, fallback) { return ptr ? UTF8ToString(ptr) : fallback; } function env(name, fallback) { if (typeof Module !== 'undefined') { if (Module['ENV'] && Module['ENV'][name]) { return Module['ENV'][name]; } if (Module[name]) { return Module[name]; } } return fallback; } const argDevice = text(device_ptr, 'qemu'); const device = env('CXL_MEMSIM_DEVICE', argDevice) || argDevice; const pool = env('CXL_MEMSIM_POOL', text(pool_ptr, 'CXLMemSim')) || 'CXLMemSim'; const size = Number(size_lo >>> 0) + Number(size_hi >>> 0) * 4294967296; const root = globalThis; const state = root.__HETGPU_CXL_BROWSER_MEMSIM || (root.__HETGPU_CXL_BROWSER_MEMSIM = { connected: false, queue: [], clientId: env('CXL_MEMSIM_CLIENT_ID', 'qemu-' + Math.random().toString(16).slice(2)), pool: pool, device: device, port: null, worker: null, requestSab: null }); state.pool = pool; state.device = device; state.poolSize = size || Number(env('CXL_MEMSIM_SIZE', 268435456)); state.clientId = env('CXL_MEMSIM_CLIENT_ID', state.clientId) || state.clientId; if (state.connected && state.port) { return 0; } if (typeof SharedWorker === 'undefined' || typeof SharedArrayBuffer === 'undefined' || typeof Atomics === 'undefined') { console.error('CXLMemSim browser transport requires SharedWorker and SharedArrayBuffer'); return -1; } let workerUrl = env('CXL_MEMSIM_WORKER_URL', env('HETGPU_CXL_MEMSIM_WORKER_URL', null)); if (!workerUrl) { const base = root.location && root.location.href ? root.location.href : 'http://127.0.0.1/'; workerUrl = new URL('/cxl2/cxlmemsim-pool-worker.js', base).href; } const workerName = env('CXL_MEMSIM_WORKER_NAME', 'hetgpu-cxlmemsim'); try { state.worker = new SharedWorker(workerUrl, workerName); state.port = state.worker.port; state.port.onmessage = (event) => { const msg = event.data || {}; if (msg.type === 'message' && msg.bytes) { state.queue.push(new Uint8Array(msg.bytes)); } else if (msg.type === 'connected' && msg.clientId) { state.clientId = msg.clientId; } else if (msg.type === 'error') { console.error('CXLMemSim worker:', msg.message || msg); } }; state.port.start(); state.port.postMessage({ type: 'connect', role: 'qemu', clientId: state.clientId, device, pool, port, size: state.poolSize || 268435456 }); state.connected = true; return 0; } catch (error) { console.error('Failed to connect CXLMemSim SharedWorker:', error); state.connected = false; state.port = null; return -1; } }
function cxl_browser_disconnect_js(device_ptr) { const state = globalThis.__HETGPU_CXL_BROWSER_MEMSIM; if (!state || !state.port) { return; } const device = device_ptr ? UTF8ToString(device_ptr) : state.device; state.port.postMessage({ type: 'disconnect', clientId: state.clientId, device: device || state.device, pool: state.pool }); }
function cxl_browser_request_js(op,addr_lo,addr_hi,size,data_ptr,value_lo,value_hi,expected_lo,expected_hi,resp_ptr,resp_size) { const state = globalThis.__HETGPU_CXL_BROWSER_MEMSIM; if (!state || !state.connected || !state.port) { return -1; } if (size > 64) { return -2; } if (typeof Atomics.wait !== 'function') { return -3; } const sab = state.requestSab || (state.requestSab = new SharedArrayBuffer(256)); const control = new Int32Array(sab, 0, 1); const bytes = new Uint8Array(sab); bytes.fill(0); Atomics.store(control, 0, 0); if ((op === 1 || op === 7) && data_ptr && size) { bytes.set(HEAPU8.subarray(data_ptr, data_ptr + size), 64); } state.port.postMessage({ type: 'sync-request', clientId: state.clientId, device: state.device, pool: state.pool, sab, op, addrLo: addr_lo >>> 0, addrHi: addr_hi >>> 0, size: size >>> 0, valueLo: value_lo >>> 0, valueHi: value_hi >>> 0, expectedLo: expected_lo >>> 0, expectedHi: expected_hi >>> 0 }); let waitResult = 'not-equal'; try { waitResult = Atomics.wait(control, 0, 0, 30000); } catch (error) { console.error('CXLMemSim browser transport cannot block on this thread:', error); return -6; } if (waitResult === 'timed-out') { return -4; } if (Atomics.load(control, 0) < 0) { return -5; } if (resp_ptr && resp_size) { HEAPU8.set(bytes.subarray(128, 128 + Math.min(resp_size, 81)), resp_ptr); } return 0; }
function cxl_browser_send_message_js(device_ptr,data_ptr,size) { const state = globalThis.__HETGPU_CXL_BROWSER_MEMSIM; if (!state || !state.connected || !state.port || !data_ptr || !size) { return -1; } const bytes = HEAPU8.slice(data_ptr, data_ptr + size); state.port.postMessage({ type: 'qemu-message', clientId: state.clientId, device: device_ptr ? UTF8ToString(device_ptr) : state.device, pool: state.pool, bytes: bytes.buffer }, [bytes.buffer]); return 0; }
function cxl_browser_recv_message_js(device_ptr,data_ptr,max_size) { const state = globalThis.__HETGPU_CXL_BROWSER_MEMSIM; if (!state || !state.queue || state.queue.length === 0) { return 0; } const bytes = state.queue.shift(); if (bytes.length > max_size) { return -1; } HEAPU8.set(bytes, data_ptr); return bytes.length; }
...
// end include: preamble.js
  ...
  allocateUnusedWorker() {
        ...
        worker = new Worker(new URL('qemu-system-x86_64-noexit1.js', import.meta.url), workerOptions);
        PThread.unusedWorkers.push(worker);
      },
  getNewWorker() {
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(7, 0, 1, fd, newfd, flags);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 0, 1, fd, mode, offset, len);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(12, 0, 1, fd, length);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(13, 0, 1, buf, size);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(14, 0, 1, fd, dirp, count);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(16, 0, 1, fd, addr, addrlen, d1, d2, d3);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(17, 0, 1, fd, level, optname, optval, optlen, d1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(18, 0, 1, fd, op, varargs);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(19, 0, 1, fd, backlog);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(20, 0, 1, path, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(21, 0, 1, dirfd, path, mode);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(22, 0, 1, dirfd, path, buf, flags);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(23, 0, 1, dirfd, path, flags, varargs);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(24, 0, 1, fdPtr);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(25, 0, 1, fds, nfds, timeout);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(26, 0, 1, dirfd, path, buf, bufsize);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(27, 0, 1, fd, buf, len, flags, addr, addrlen);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(28, 0, 1, fd, message, flags, d1, d2, d3);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(29, 0, 1, olddirfd, oldpath, newdirfd, newpath);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(30, 0, 1, path);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(32, 0, 1, fd, message, length, flags, addr, addr_len);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(33, 0, 1, domain, type, protocol);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(34, 0, 1, path, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(35, 0, 1, path, size, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(36, 0, 1, target, linkpath);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(38, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(39, 0, 1, addr, len, prot, flags, fd, offset);
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */
    cxl_browser_connect_js,
    /** @export */
    cxl_browser_disconnect_js,
    /** @export */
    cxl_browser_recv_message_js,
    /** @export */
    cxl_browser_request_js,
    /** @export */
    cxl_browser_send_message_js,
    /** @export */
...
var dynCall_iijiiiji = Module['dynCall_iijiiiji'] = createExportWrapper('dynCall_iijiiiji', 8);
...
var dynCall_iijiiiii = Module['dynCall_iijiiiii'] = createExportWrapper('dynCall_iijiiiii', 8);
...
var dynCall_jijii = Module['dynCall_jijii'] = createExportWrapper('dynCall_jijii', 5);
...
var dynCall_iiijijjii = Module['dynCall_iiijijjii'] = createExportWrapper('dynCall_iiijijjii', 9);
var dynCall_viiiijjii = Module['dynCall_viiiijjii'] = createExportWrapper('dynCall_viiiijjii', 9);
...
var dynCall_jijjji = Module['dynCall_jijjji'] = createExportWrapper('dynCall_jijjji', 6);
...
var dynCall_jijji = Module['dynCall_jijji'] = createExportWrapper('dynCall_jijji', 5);
var dynCall_iiiiiiijiiiii = Module['dynCall_iiiiiiijiiiii'] = createExportWrapper('dynCall_iiiiiiijiiiii', 13);
...
var dynCall_vijjjj = Module['dynCall_vijjjj'] = createExportWrapper('dynCall_vijjjj', 6);
var dynCall_jij = Module['dynCall_jij'] = createExportWrapper('dynCall_jij', 3);
var dynCall_viijij = Module['dynCall_viijij'] = createExportWrapper('dynCall_viijij', 6);
...
var dynCall_iiijjiii = Module['dynCall_iiijjiii'] = createExportWrapper('dynCall_iiijjiii', 8);
var dynCall_iijjiii = Module['dynCall_iijjiii'] = createExportWrapper('dynCall_iijjiii', 7);
