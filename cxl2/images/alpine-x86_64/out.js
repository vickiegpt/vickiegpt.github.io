...
var ASM_CONSTS = {
  ...
  9883470: ($0, $1, $2, $3, $4, $5, $6, $7, $8) => {
    if (typeof console !== "undefined") {
      var n = UTF8ToString($1);
      console.log("[hetGPU WebGPU] launch kernel=" + $0 + " name=" + n + " grid=(" + $2 + "," + $3 + "," + $4 + ")" + " block=(" + $5 + "," + $6 + "," + $7 + ")" + " shared=" + $8);
    }
  }
};
...
function ___syscall_fallocate(fd, mode, offset, len) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 0, 1, fd, mode, offset, len);
...
function ___syscall_pipe(fdPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 0, 1, fdPtr);
  fdPtr >>>= 0;
  try {
    if (fdPtr == 0) {
...
function ___syscall_symlink(target, linkpath) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(36, 0, 1, target, linkpath);
  target >>>= 0;
  linkpath >>>= 0;
  try {
